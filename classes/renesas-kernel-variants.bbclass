...
python () {
    ...
    fields = ["TREE", "PRETTY", "PATCH", "PATCH_LOCATIONS", "CONFIG", "IMAGE", "SYMLINK"]
...
renesas_kernel_compile_variant() {
    ...
    patch_locations=$(renesas_kernel_variant_get "$variant" PATCH_LOCATIONS)
    ...
    bbnote "Building ${pretty} kernel variant"

    if [ -d "${S}/.git" ]; then
        git -C "${S}" worktree remove --force "${tree}" 2>/dev/null || true
    ...
    if [ -d "${S}/.git" ]; then
        bbnote "Creating git worktree at ${tree} for ${pretty} build"
        if ! git -C "${S}" worktree add --force --detach "${tree}" HEAD 2>&1; then
            bbnote "git worktree failed; falling back to rsync copy"
            rsync -a --delete --exclude='.git' "${S}/" "${tree}/"
        fi
    else
        bbnote "Copying kernel source to ${tree} (no .git found)"
        rsync -a --delete --exclude='.git' "${S}/" "${tree}/"
    fi
    ...
    if [ -n "$patch_name" ]; then
        ...
        if [ -f "$patch_name" ]; then
            ...
            for candidate in $patch_locations; do
                if [ -f "$candidate" ]; then
                    patch_file="$candidate"
                    break
                fi
            done
            if [ -z "$patch_file" ] && [ -f "${WORKDIR}/${patch_name}" ]; then
                patch_file="${WORKDIR}/${patch_name}"
            fi
        fi

        if [ -n "$patch_file" ]; then
            bbnote "Applying ${pretty} patch from ${patch_file}"
            set +e
            patch -d "${tree}" -p1 --forward --reject-file=- < "${patch_file}"
            rc=$?
            set -e
            if [ $rc -ne 0 ]; then
                bbnote "Standard patch failed (rc=$rc); trying git am"
                ( cd "${tree}" && git init && git config user.email "builder@example.com" && \
                    git config user.name "Yocto Builder" && git add -A && git commit -m init && \
                    git am -3 --keep-cr "${patch_file}" ) || \
                    bbfatal "Failed to apply patch ${patch_file} via patch and git am"
            fi
        else
            bbnote "No patch applied for ${pretty} (patch '${patch_name}' not found)"
        fi
    fi
    ...
    oe_runmake -C "${tree}" olddefconfig
    oe_runmake -C "${tree}" ${KERNEL_IMAGETYPE}
    oe_runmake -C "${tree}" modules
}
