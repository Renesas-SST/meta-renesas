...
function chelp() {
	...
	echo "the default location is set to renesas-core-image-weston-rzg2l-sbc.tar.bz2 inside any release package"
	echo "------------------------------------------------------"
}

# Default core-image
rootfs_image_path="../../../../target/images/rootfs/renesas-core-image-weston-rzg2l-sbc.tar.bz2"
if [ $# -eq 0 ]
  then
...
then
	...
	then
		...
		ls -l
		echo "Copying environment and binary files in the boot directory to partition 1"
		cp -rL ./* /tmp/rz_sdm1
		echo "Syncing fs"
		sync /tmp/rz_sdm1
		sync /tmp/rz_sdm2
